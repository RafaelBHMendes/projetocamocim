...
const LicitacoesPage: React.FC = () => {
  ...
  const handleButtonClick = (buttonType: 'Avisos' | 'Licitações' | 'Dispensas' | 'Pregão') => {
  ...
  const filterBiddings = () => {
    if (activeButton === 'Avisos') {
      ...
      setFilteredBiddings(biddings.filter(bidding => bidding.modality === 'Dispensa'))
    } else if (activeButton === 'Pregão') {
      setFilteredBiddings(biddings.filter(bidding => bidding.modality === 'Pregão'))
